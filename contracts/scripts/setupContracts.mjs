...
async function setupSource(chainId, deployedContracts, destinationChainID) {
  ...
  const ism = deployedContracts[chainId].Contracts.OracleRequestRecipient;
  ...
  console.log("to add as owner",requestRecipientAddress);

  if (!owners.includes(requestRecipientAddress)) {
  tx = await oracleTriggerFactory.addOwner(requestRecipientAddress)
  await tx.wait();
  console.log(
    ...
    console.log("requestRecipientAddress is already owner")
}
}
