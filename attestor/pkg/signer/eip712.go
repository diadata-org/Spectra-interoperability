...
func (s *EIP712Signer) VerifySignature(ctx context.Context, signedIntent []byte) (bool, error) {
	...
	if err := json.Unmarshal(signedIntent, &data); err != nil {
		return false, errors.NewSignerError("parse signed intent", "", err)
	}
	
	// Extract signature and signer
	_, ok := data["signature"].(string)
	if !ok {
		return false, errors.NewSignerError("verify", "missing signature", nil)
	}
	
	signer, ok := data["signer"].(string)
	if !ok {
		return false, errors.NewSignerError("verify", "missing signer", nil)
	}
	
	// Check if signer matches our address
	if !strings.EqualFold(signer, s.address.Hex()) {
		return false, nil
	}
	
	// TODO: Implement actual signature verification
	// This would require reconstructing the EIP-712 typed data hash
